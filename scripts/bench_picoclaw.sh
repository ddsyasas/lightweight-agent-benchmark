#!/usr/bin/env bash
# usage: ./scripts/bench_picoclaw.sh <task>   e.g. ./scripts/bench_picoclaw.sh task2
# Mirrors bench.sh (zeroclaw) but for PicoClaw. Sessions are cleared before each
# trial so trials are independent (matches zeroclaw's load_session_context=false).
set -u
